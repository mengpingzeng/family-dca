#!/usr/bin/env python3
"""
Wind 数据策略验证 — 严格买入统一最优策略 (B8/12/22/40 S75/85)

用单纯 wind PE (pe_ttm_wind) 重算 10 年滚动百分位后, 套用严格买入策略验证。

输出: latest_validate_wind.json (供前端)
"""
import os, sys, json
...
MERGED_DIR = os.path.abspath(os.path.join(OUTPUT_DIR, os.pardir, os.pardir, "data-store", "parquet", "aks_merged_wind"))
...
WIND_INDICES = [
    ...
    {"code": "930930", "name": "港股综合", "market": "港股通"},
    {"code": "930931", "name": "港股通50", "market": "港股通"},
...
def main():
    print("=" * 60)
    print("Wind 数据策略验证 — B8/12/22/40 S75/85 (10年窗口)")
    print("=" * 60)

    summary = []
    for target in WIND_INDICES:
        code = target["code"]
        name = target["name"]
    ...
    out = {
        ...
        "window": WINDOW,
        "source": "wind (pe_ttm_wind)",
        "summary": summary,
    }

    out_path = os.path.join(OUTPUT_DIR, "latest_validate_wind.json")
