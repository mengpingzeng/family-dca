#!/usr/bin/env python3
"""
ETF 本金阈值回测 — 自定义交易起点版 (策略 B + 本金阈值 vs 策略 B 原版, 12 只真实 ETF)。

与 test_etf_capped.py 完全相同的策略 / 本金阈值 / 费用模型, 仅交易起点改为命令行指定日期,
终点为数据最后一天 (今天)。

百分位窗口: 统一按「最多10年历史、不足则有多少用多少」规则 — 窗口封顶10年,
最少20样本即开始交易 (不再按各指数原 10/5/3 满窗口卡起点)。

用法: python wind_new_search/test_etf_capped_since.py --start 2007-10-16
输出: wind_new_search/output/test_etf_capped_since_20071016.json  (不影响原版结果)
"""
import argparse
import json
import sys
...
def main():
    ...
    ap.add_argument("--start", required=True, help="交易起点, 如 2007-10-16")
    args = ap.parse_args()
    ...
    for idx_code, (etf_code, etf_name) in ETF_MAP.items():
        df, cfg = build_windowed_df(idx_code, start_date=start_date, uniform10yr=True)
        if df is None:
        ...
        if len(df_etf) == 0:
            print(f"[跳过] {idx_code} 起点 {start_date} 之前 ETF {etf_code} 尚无数据")
            continue
        ...
        first_trade = r_base["cash_flows"][0][0] if r_base["cash_flows"] else None
        results.append({
            "code": idx_code, "name": NAMES.get(idx_code, idx_code),
            "etf_code": etf_code, "etf_name": etf_name,
            "window": cfg["window"],
            "start_date": first_trade or str(df_etf["date"].min().date()),
            "etf_data_start": str(df_etf["date"].min().date()),
    ...
    out_path = OUTPUT_DIR / f"test_etf_capped_since_{tag}.json"
