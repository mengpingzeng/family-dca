...
从蛋卷基金 API 获取指数 PB（市净率）历史数据（周频、等权PB），保存为 Parquet 文件。

与 PE 下载器独立运行，输出到 parquet/index_pb/，互不冲突。

用法:
    python download_danjuan_pb.py                  # 下载全部13个指数
    python download_danjuan_pb.py --check           # 验证接口连通性
    python download_danjuan_pb.py --list            # 列出所有指数
    python download_danjuan_pb.py --code 000300     # 下载指定指数
"""
...
DJ_API_BASE = "https://danjuanfunds.com/djapi/index_eva/pb_history"
...
def fetch_danjuan_pb(danjuan_code: str) -> pd.DataFrame:
    """
    从蛋卷基金获取单个指数的 PB 历史数据（周频）。

    Args:
        danjuan_code: 蛋卷格式代码，如 "SH000300"

    Returns:
        DataFrame(date, pb_dj)，按日期升序排列。空DataFrame表示无数据。
    """
    ...
    pb_items = data["data"].get("index_eva_pb_growths", [])
    if not pb_items:
        return pd.DataFrame()

    df = pd.DataFrame(pb_items)
    df["date"] = pd.to_datetime(df["ts"], unit="ms").dt.date
    df["pb_dj"] = pd.to_numeric(df["pb"], errors="coerce")
    df = df[["date", "pb_dj"]].dropna(subset=["pb_dj"])
...
def download_index(index_info: dict, output_dir: str) -> bool:
    ...
    pb_min = df["pb_dj"].min()
    pb_max = df["pb_dj"].max()
    pb_latest = df.iloc[-1]["pb_dj"]

    print(f"OK | {len(df)}条 | {date_min} ~ {date_max} | "
          f"PB {pb_min:.2f}~{pb_max:.2f} | 最新 {pb_latest:.4f}")
...
def download_all(output_dir: str):
    ...
    print(f"\n蛋卷PB数据下载 — {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    ...
    for idx in skipped:
        print(f"[SKIP] {idx['code']} {idx['name']} — 蛋卷无此指数PB")
...
def check():
    ...
    df = fetch_danjuan_pb(idx["dj_code"])
    if df.empty:
    ...
    print(f"  PB范围:   {df['pb_dj'].min():.4f} ~ {df['pb_dj'].max():.4f}")
    print(f"  最新PB:   {df.iloc[-1]['pb_dj']:.4f} ({df.iloc[-1]['date']})")
...
def main():
    ...
    parser.add_argument(
        ...
        help="输出目录（默认 data-store/parquet/index_pb/）"
    )
    args = parser.parse_args()

    output_dir = args.output or os.path.join(PROJECT_DIR, "data-store", "parquet", "index_pb")
